...
#read in data
carnaval_data <- read.csv("Carnaval_et_al_GPS.csv")
```
...
carnaval_data2 <- subset(carnaval_data, select = c("Species_name", "Lat",	"Lon")) %>%
  subset(Lat < 12.5) %>%
  subset(Lon < -30 & Lon > -100)  %>%
  na.omit() %>%
  droplevels.data.frame()

```

The original number of observations was 734, and the filtered number of observations is 734, including 26 species.
...
coord_points <- st_as_sf(carnaval_data2, coords = c("Lon", "Lat"), crs = 4326, agr = "constant")
...
100x100 km. Dataset is reduced from 26 to 19 species and from 734 observations to 261 observations. (species: length(unique(pts_ext_1d$Species_name)) / observations:str(pts_ext_1d))
```{r}
...
200x200 km. 26 to 24 species and from 734 observations to 454 observations. (species: length(unique(pts_ext_2d$Species_name)) / observations:str(pts_ext_2d))
...
300x300 km. 26 to 26 species and from 734 observations to 557 observations. (species: length(unique(pts_ext_3d$Species_name)) / observations:str(pts_ext_3d))

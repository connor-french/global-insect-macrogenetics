...
#read in data
total_data <- read.csv("TOTAL_GPS_(Smith+Miraldo+Carnaval).csv")
```
...
The original number of observations was 14959, and the filtered number of observations is 1097, including 234 species.
...
100x100 km. Dataset is reduced from 2595 to 347 species and from 53729 observations to 13986 observations. (species: length(unique(pts_ext_1d$Species_name)) / observations:str(pts_ext_1d))
```{r}
#create a numeric color palette so I can bin the colors (doing this up front so I don't repeatedly assign this variable)
pal2 <- colorBin(palette = "inferno", domain =NULL, bins= c(3,5,10,20,50,100,200,500,3600), na.color = "#00000000")
...
200x200 km. 2595 to 366 species and from 53729 observations to 14887 observations. (species: length(unique(pts_ext_2d$Species_name)) / observations:str(pts_ext_2d))
...
300x300 km. 2595 to 381 species and from 53729 observations to 15353 observations. (species: length(unique(pts_ext_3d$Species_name)) / observations:str(pts_ext_3d))
...
100 km. 
```{r}
pal3 <- colorBin(palette = "inferno", domain =NULL, bins= c(1,3,5,10,20,30,40,50,60,80,100), na.color = "#00000000")

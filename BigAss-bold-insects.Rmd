...
count_pts_1d_sp <- rasterize(pts_ext_1d_sp, sa_clim_1d, fun = function(x, ...) {length(unique(x))}, field = "bin_uri") #make a raster out of the counts of species per cell
...
test.nuc <- pts_ext_1d %>% 
  ...
  ungroup() %>%
  drop.levels()
fwrite(test.nuc, file = "../bold-seqs-insects-10.txt")
test.nuc <- fread("bold-seqs-insects-10.txt")
...
dir.create("../bold-seqs-insects-10")
...
nexus.write.fun <- function(x) {
  for (i in seq_along(x)) {
    ...
    for (j in seq_along(df)) {
      ...
      write.nexus.data(nuc.pad, file = paste0("../bold-seqs-insects-10/", str_replace(unique(nuc.df[,2]), " ", "-"), ".", unique(nuc.df[,3]), ".nex"), missing = "N", interleaved = FALSE) #write to a nexus file, which is named by the species and cell number
    }
  }
}
...
files <- paste0("../bold-seqs-insects-10/", list.files("../bold-seqs-insects-10"))
...
write.csv(pi.df.one, file = "raw-pi-insects-10.csv")
pi.df.one <- fread("raw-pi-insects-10.csv")
...
write.csv(sum.df, file = "pi-summary-insects-10.csv")
sum.df <- fread("pi-summary-insects-10.csv")
...
pi.plot <- merge(test.nuc, sum.df, by = "cells") %>% 
  ...
  dplyr::select(recordID, bin_uri, cells, marker_codes, median.pi.avg, median.pi.sd, mean.pi.avg, mean.pi.sd, hill.zero.avg, hill.zero.sd, hill.one.avg, hill.one.sd, hill.two.avg, hill.two.sd, shannon.avg, shannon.sd, Long, Lat, contains("bio_10m")) #%>%

---
title: "BigAss Amphibian phylogeography data exploration"
output:
  html_document:
    df_print: paged
---
Here are some maps exploring the amphibian data. I have two sets of maps: Maps only with cells where there are at least three individuals per species (>= 3 species tab), and maps that show the species density, rather than observation density (Species density tab). 
...
#read in data
total_data <- read.csv("TOTAL_GPS_Amphibians.csv")
```
...
100x100 km. Dataset is reduced from 845 to 65 species and from 9909 observations to 1560 observations. (species: length(unique(pts_ext_1d$Species_name)) / observations:str(pts_ext_1d))
```{r}
#create a numeric color palette so I can bin the colors (doing this up front so I don't repeatedly assign this variable)
pal2 <- colorBin(palette = "inferno", domain =NULL, bins= c(3,5,10,20,50,100,500,1560), na.color = "#00000000")
...
200x200 km. 845 to 72 species and from 9909 observations to 1451 observations. (species: length(unique(pts_ext_2d$Species_name)) / observations:str(pts_ext_2d))
...
300x300 km. 845 to 79 species and from 9909 observations to 1559 observations. (species: length(unique(pts_ext_3d$Species_name)) / observations:str(pts_ext_3d))
...
100 km. 
```{r}
pal3 <- colorBin(palette = "inferno", domain =NULL, bins= c(1,3,5,8,10), na.color = "#00000000")

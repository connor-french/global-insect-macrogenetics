---
title: "BigAss Mammal phylogeography data exploration"
output:
  html_document:
    df_print: paged
---
Here are some maps exploring the mammal data. I have two sets of maps: Maps only with cells where there are at least three individuals per species (>= 3 species tab), and maps that show the species density, rather than observation density (Species density tab). 
...
#read in data
total_data <- read.csv("TOTAL_GPS_Mammals.csv")
```
...
100x100 km. Dataset is reduced from 1604 to 210 species and from 36412 observations to 10350 observations. (species: length(unique(pts_ext_1d$Species_name)) / observations:str(pts_ext_1d))
```{r}
#create a numeric color palette so I can bin the colors (doing this up front so I don't repeatedly assign this variable)
pal2 <- colorBin(palette = "inferno", domain =NULL, bins= c(3,5,10,50,100,500,10350), na.color = "#00000000")
...
200x200 km. 1604 to 219 species and from 36412 observations to 10705 observations. (species: length(unique(pts_ext_2d$Species_name)) / observations:str(pts_ext_2d))
...
300x300 km. 1604 to 223 species and from 36412 observations to 10820 observations. (species: length(unique(pts_ext_3d$Species_name)) / observations:str(pts_ext_3d))
...
100 km. 
```{r}
pal3 <- colorBin(palette = "inferno", domain =NULL, bins= c(1,3,5,10,20,50,210), na.color = "#00000000")

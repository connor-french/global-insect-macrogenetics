...
#read in data
mammals_COI_data <- read.csv("Miraldoetal_GPS-mammals_COI.csv")
```
...
mammals_COI_data2 <- subset(mammals_COI_data, select = c("Species_name", "Latitude",	"Longitude")) %>%
...
The original number of observations was 22762, and the filtered number of observations is 10854, including 282 species.
...
coord_points <- st_as_sf(mammals_COI_data2, coords = c("Longitude", "Latitude"), crs = 4326, agr = "constant")
...
100x100 km. Dataset is reduced from 282 to 172 species and from 10854 observations to 9854 observations. (species: length(unique(pts_ext_1d$Species_name)) / observations:str(pts_ext_1d))
```{r}
...
200x200 km. 282 to 179 species and from 10854 observations to 10165 observations. (species: length(unique(pts_ext_2d$Species_name)) / observations:str(pts_ext_2d))
...
300x300 km. 282 to 184 species and from 10263 observations to 616 observations. (species: length(unique(pts_ext_3d$Species_name)) / observations:str(pts_ext_3d))

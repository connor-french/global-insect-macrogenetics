...
r2_gdm_plot <- ggplotGrob(ggplot(data = r2_df_gdm, aes(x = post)) +
  geom_density(color = col_gdm, size = 1) +
...
r2_gde_plot <- ggplotGrob(ggplot(data = r2_df_gde, aes(x = post)) +
  geom_density(color = col_gde, size = 1) +
...
## Model output
```{r}
gdm_tables <- map(model_gdm_full, glmmfields::tidy)

gde_tables <- map(model_gde_full, glmmfields::tidy)
```

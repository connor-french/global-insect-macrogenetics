...
```{r, write-2d-scatter-predict}
ggsave(filename = here("output", "publication_figs", "map_2d_scatter_predict.svg"),
       plot = map_2d_scatter,
       width = 36,
       height = 22.5,
       dpi = 300,
...
```{r, write-2d-scatter-observed}
ggsave(filename = here("output", "publication_figs", "map_2d_scatter_observed.svg"),
       plot = obs_2d_scatter,
       width = 36,
       height = 22.5,
       dpi = 300,
...
model_data <- read_sf(here("output", "spreadsheets", "full_sf.geojson"), crs = crs_behr) %>% 
  filter(num_otu >= 100) %>% 
  # transform to regular lat-long for recognizable coordinates
  st_transform(crs = 4326)

model_coords <- st_coordinates(st_centroid(model_data))

model_data <- model_data %>% 
  mutate(longitude = model_coords[,1],
         latitude = model_coords[,2])
...
```{r}
lat_gdm <- ggplot(data = model_data, aes(x = abs(latitude), y = gdm)) +
  geom_point(size = 3) +
...
```{r, write-lat-gdm}
ggsave(filename = here("output", "publication_figs", "latitude_scatter_gdm.svg"),
       plot = lat_gdm,
       width = 18,
       height = 11.25,
       dpi = 300,
...
```{r}
lat_smooth_gde <- ggplot(data = model_data, aes(x = abs(latitude), y = gde)) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", formula = y ~ x + I(x^2), color = col_gde, se = FALSE, size = 2) +
...
```{r, write-lat-gde}
ggsave(filename = here("output", "publication_figs", "latitude_scatter_gde.svg"),
       plot = lat_smooth_gde,
       width = 18,
       height = 11.25,
       dpi = 300,
...
```{r}
lat_combo <- (lat_smooth_gdm + lat_gdm) / (lat_smooth_gde + lat_gde) + plot_annotation(tag_levels = "a", tag_suffix = ")") 

...
#read in data
mammals_CytB_data <- read.csv("Miraldoetal_GPS-mammals_CytB.csv")
```
...
mammals_CytB_data2 <- subset(mammals_CytB_data, select = c("Species_name", "Latitude",	"Longitude")) %>%
...
coord_points <- st_as_sf(mammals_CytB_data2, coords = c("Longitude", "Latitude"), crs = 4326, agr = "constant")
...
100x100 km. Dataset is reduced from 234 to 60 species and from 1097 observations to 562 observations. (species: length(unique(pts_ext_1d$Species_name)) / observations:str(pts_ext_1d))
```{r}
...
200x200 km. 234 to 66 species and from 1097 observations to 601 observations. (species: length(unique(pts_ext_2d$Species_name)) / observations:str(pts_ext_2d))
...
300x300 km. 234 to 65 species and from 1097 observations to 616 observations. (species: length(unique(pts_ext_3d$Species_name)) / observations:str(pts_ext_3d))

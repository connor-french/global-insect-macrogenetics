...
Write the sequence data in the BOLD csv to nexus files. Writing all species with greater than 3 individuals per file to a nexus file. I will filter for cells with different numbers of species per cell later. Each nexus file is labeled as "speciesname.cell.nex".
```{r}
#####split 
test.nuc <- subset(pts_ext_1d, select = c(recordID, species_name, cells, marker_codes, nucleotides)) %>% 
  as.data.frame() %>%
  ...
  filter(n() > 9) %>% #retain only observations where there are more than two species observations per cell
  drop.levels()
...
nexus.write.fun <- function(x) {
  for (i in seq_along(x)) {
    ...
    for (j in seq_along(df)) {
      ...
      write.nexus.data(nuc.pad, file = paste0("../bold-seqs/", str_replace(unique(nuc.df[,2]), " ", "-"), ".", unique(nuc.df[,3]), ".nex"), missing = "N", interleaved = FALSE) #write to a nexus file, which is named by the species and cell number
    }
  }
}
...
files <- paste0("../bold-seqs/", list.files("../bold-seqs"))
...
#function to only extract cell number from the name of the sequence. Some species names include a number, so I need to extract only the second number grouping
...
#####Next project- figure out how to effectively filter after calculating pi


pi.medians.one <- pi.df.one %>% group_by(cells) %>% dplyr::summarise(median.pi = median(pi), hill.num = hill.calc(pi, 1), shannon = entropy::entropy(pi)) #calculate pi for each cell number. I'm using dplyr::summarise because plyr also has a summarise function that can create a lot of confusion.

pi.no.na <- na.omit(pi.medians.one)

pts.ext.one.pi <- merge(pts_ext_1d, pi.means.no.na, by = "cells") #make new spatial data frame including the pi values


pi.raster.one <- rasterize(pts.ext.one.pi, sa_clim_1d, fun = "first", field = "mean.pi") #create new raster of mean pi values per cell

pal.one <- pal.phylum <- colorBin(palette = "inferno", bins = seq(0, max(na.omit(pts.ext.one.pi$mean.pi)), length.out = 10), domain = NULL, pretty = TRUE, na.color = "#00000000")

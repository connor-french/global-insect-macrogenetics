...
pts_ext_1d_sp <- as.data.frame(pts_ext_1d) %>% #convert to data frame for filtering
  group_by(cells) %>%
  filter(n_distinct(species_name) > 5) %>%
  ungroup()
...
test.nuc <- pts_ext_1d %>% 
  ...
  dplyr::select(recordID, species_name, cells, marker_codes, nucleotides, Lat, Long, contains("bio_10m")) %>% 
...
sum.list<- replicate(1000, pi.summary.fun(pi.df.one, 3), simplify = FALSE) 
...
write.csv(sum.df, file = "pi-summary-3.csv")
...
pi.plot <- merge(test.nuc, sum.df, by = "cells") %>% 
  ...
  dplyr::select(recordID, species_name, cells, marker_codes, median.pi.avg, median.pi.sd, mean.pi.avg, mean.pi.sd, hill.zero.avg, hill.zero.sd, hill.one.avg, hill.one.sd, hill.two.avg, hill.two.sd, shannon.avg, shannon.sd, Long, Lat, contains("bio_10m")) %>%
...
mars.data <- pi.plot %>% 
  ...
  dplyr::select(median.pi.avg, contains("bio_10m")) %>% 
  na.omit()
...
mars.training.response <- mars.training %>% dplyr::select(median.pi.avg) %>% as.matrix() %>% as.numeric()
...
#select mean pi as response
mars.test.response <- mars.test %>% dplyr::select(median.pi.avg) %>% as.matrix() %>% as.numeric()
...
mars.grid <- expand.grid(degree = 1:2, nprune = seq(2, 19, by = 1))
...
```{r}
mars.test.cv <- mars.test %>% 
  as_tibble() %>% 
  mutate(pred = as.vector(mars.pred.gcv))
rmse(mars.test.cv, truth = response, estimate = pred)
rsq(mars.test.cv, truth = response, estimate = pred)
```
